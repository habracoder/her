...
var AppClass = function() {
    this.loaderImg = '/img/ajax_loader.gif';
    Mods.import('ambry');
};
 ...
 */
AppClass.prototype.ajaxLoading = function(selector, type) {
    selector = selector != undefined ? selector : 'body';
    type = type != undefined ? type : '';
    var element = $('.ajaxOverlay' + type);
    ...
    $(selector).append($('<img />', {
        'src': this.loaderImg,
        'id': 'loaderGif',
        'class': 'ajaxLoader' + type
    }));

    if ($(selector)[0].tagName == 'BODY') {
        $('.ajaxLoader' + type).css('position', 'fixed');
...
/**
 * Прятает загрузчик и затмение
 */
AppClass.prototype.hideOverlayAndLoader = function() {
